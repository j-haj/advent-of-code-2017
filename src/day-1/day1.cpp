...
int matching_sum(const std::string& digits) {
  ...
  for (size_t i = 0; i < n; ++i) { 
    if (i < n - 1 && (digits[i] == digits[i+1])) sum += digits[i] - '0';
    else if (i == n - 1 && (digits[n-1] == digits[0])) sum += digits[n-1] - '0';
  }
  return sum;
}

void run_tests() {
  std::unordered_map<std::string, int> tests = {{"1111", 4},
    {"1234", 0}, {"91212129", 9}, {"1122", 3}};
